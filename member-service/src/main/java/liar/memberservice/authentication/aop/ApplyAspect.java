...
public class ApplyAspect {
    ...
    public static class AllAuthentication {
        ...
        @Before("liar.memberservice.authentication.aop.Pointcuts.allAuthentication()")
        public void beforeAuthentication(JoinPoint joinPoint) throws Throwable {
            log.info("[]");
        }

        @AfterReturning(value = "liar.memberservice.authentication.aop.Pointcuts.allAuthentication()", returning = "result")
        public void allReturn(JoinPoint joinPoint, Object result) {
            log.info("[AfterReturning AllReturn Result] result = {}", result.toString());
        }

        @After(value = "liar.memberservice.authentication.aop.Pointcuts.allAuthentication()")
