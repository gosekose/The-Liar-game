...
class WaitRoomRedisRepositoryTest2 {

    @Autowired
    CrudCustomRepository<WaitRoom, String> crudCustomRepository;
...
//    }
    ...
    public void saveWaitRoom() throws Exception {
        ...
        crudCustomRepository.set(waitRoom);

        //when
        WaitRoom findRoom = crudCustomRepository.get(waitRoom.getId(), WaitRoom.class);

        //then
        assertThat(findRoom.getRoomName()).isEqualTo("koseTest1");
        assertThat(findRoom.getHostId()).isEqualTo("kose");
        assertThat(findRoom.getLimitMembers()).isEqualTo(5);
        assertThat(findRoom.getMembers().size()).isEqualTo(1);
    }
//
//    @Test
